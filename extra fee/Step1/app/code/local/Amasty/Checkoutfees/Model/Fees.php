...
class Amasty_Checkoutfees_Model_Fees extends Mage_Rule_Model_Rule
{
    ...
    {
        $value = $this->getData(self::DISCOUNT_IN_SUBTOTAL);

        if ($value === Amasty_Checkoutfees_Model_Options_Excludeinclude::VAR_DEFAULT){
            $value = Mage::getStoreConfig(
                'amcheckoutfees/calculation/discount_in_subtotal'
            );
        }
    ...
    {
        $value = $this->getData(self::TAX_IN_SUBTOTAL);

        if ($value === Amasty_Checkoutfees_Model_Options_Excludeinclude::VAR_DEFAULT){
            $value = Mage::getStoreConfig(
                'amcheckoutfees/calculation/tax_in_subtotal'
            );
        }
    ...
    {
        $value = $this->getData(self::SHIPPING_IN_SUBTOTAL);

        if ($value === Amasty_Checkoutfees_Model_Options_Excludeinclude::VAR_DEFAULT){
            $value = Mage::getStoreConfig(
                'amcheckoutfees/calculation/shipping_in_subtotal'
            );
        }
